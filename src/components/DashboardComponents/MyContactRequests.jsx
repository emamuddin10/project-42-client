...
const MyContactRequest = () => {
  ...
  const fetchContactRequest = async () => {
    const { data } = await axios.get(`https://project-42-server.onrender.com/contact-requests-info/${user.email}`);
    return data;
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://project-42-server.onrender.com/contact-requests-info/${id}`)
          .then(() => {
            Swal.fire({
              title: "Deleted!",
